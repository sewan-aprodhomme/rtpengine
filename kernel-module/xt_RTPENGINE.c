		...
		struct re_address *dst, uint8_t in_tos, const struct xt_action_param *par)
{
	...
	if (rtp.ok) {
		...
		if (srtp_decrypt(&g->decrypt_rtp, &g->target.decrypt, &rtp, pkt_idx))
			goto out_error;

		skb_trim(skb, rtp.header_len + rtp.payload_len);

		if (g->target.rtp_stats && ssrc_idx >= 0)
		...
		pkt_idx = 0;
		printk(KERN_WARNING "XXXXXX handling RTCP");
	...
	for (i = 0; i < g->target.num_destinations; i++) {
		...
		err = send_proxy_packet_output(skb2, g, o, &rtp2, ssrc_idx, par);
		if (err)
			atomic64_inc(&g->stats.errors);
	}
