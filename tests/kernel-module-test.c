...
#define MSGLEN(op, type, buf, len, args...) do { \
	printf("exec %s:%i\n", __FILE__, __LINE__); \
	struct rtpengine_message_ ## type *rm = malloc(sizeof(*rm) + len); \
	*rm = (struct rtpengine_message_ ## type) { .cmd.cmd = op, .type = { args } }; \
	if (len) { \
		void *data = (void *) rm + sizeof(*rm); \
		memcpy(data, buf, len); \
	} \
	ssize_t ret = write(fd, rm, sizeof(*rm) + len); \
...
#define MSG(op, type, args...) MSGLEN(op, type, NULL, 0, args)
#define MSGBUF(op, type, buf, args...) MSGLEN(op, type, buf, LEN(buf), args)
...
int main(void) {
	...
	MSG(REMG_NOOP, noop,
			.noop_size = sizeof(struct rtpengine_message_noop),
			.target_size = sizeof(struct rtpengine_message_target),
			.destination_size = sizeof(struct rtpengine_message_destination),
